/**
 * EXO Wave B — Holding metrics: durable North Star events feeding a single
 * "dual mission score" that tries to answer, at a glance, whether the holding
 * layer is routing capital *and* healing people, not just one or the other.
 */
...
export async function recordMetric(db: DatabaseClient, input: RecordMetricInput): Promise<HoldingMetricEvent> {
  const result = await db.query<Record<string, unknown>>(
    `INSERT INTO rarecrest.holding_metric_events
       (vertical, entity_id, metric_key, value_numeric, unit, source_ref, actor_id)
